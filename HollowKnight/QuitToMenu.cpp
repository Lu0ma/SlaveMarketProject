...
#define PATH_QUIT_GAME_MENU "UIs/Menus/QuitToMenu/Background.png"

QuitToMenu::QuitToMenu(Menu* _owner) : Menu("QuitToMenu", _owner)
{
	buttons = vector<Button*>();
}


void QuitToMenu::Init()
{
	...
	const vector<ButtonData>& _allData = {
		...
		ButtonData("No", [&]() {
			SetStatus(false);
			dynamic_cast<PauseMenu*>(owner)->SetStatus(true);
		}),
	};
