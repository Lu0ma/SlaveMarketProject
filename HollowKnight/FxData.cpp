...
FxData::FxData(Actor* _actor, const string _path)
	: IManagable(_path)
{
	animation = new AnimationComponent(_actor);
...
void FxData::Init()
{
	...
	animation->InitAnimations({
		...
		AnimationData("FxMana", Vector2f(0.0f, 91.0f), Vector2f(510.0f, 301.0f), READ_RIGHT, false, 4, 0.05f),
		AnimationData("FxSpecial", Vector2f(0.0f, 440.0f), Vector2f(340.0f, 321.0f), READ_RIGHT, false, 3, _speed),
		AnimationData("FxDash", Vector2f(0.0f, 800.0f), Vector2f(383.0f, 342.0f), READ_RIGHT, false, 6, 0.05f),
		AnimationData("FxDoubleJump", Vector2f(0.0f, 1253.0f), Vector2f(487.0f, 422.0f), READ_RIGHT, false, 3, _speed),
		});
}
