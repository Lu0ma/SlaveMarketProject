...
#pragma endregion
	...
	if (shake.current >= shake.max) return;
		double _angle = CAMERA_SHAKE_ANGLE * shake.trauma * Randn();
		Vector2f _offset;
...
void Camera::Down()
{
	...
	if (IsAtDestination(_distance)) return;
	const float _offsetX = Game::GetPlayer()->GetDrawable()->getScale().x > 0.0f ? offset.x : -offset.x;
	targetPosition = Vector2f(_player->GetShapePosition() + Vector2f(0.0f, 500.0f));
	Vector2f _direction = targetPosition - view.getCenter();
	Normalize(_direction);
	view.move(_direction * (speed  + 0.5f)  * _deltaTime * abs(_distance * 0.01f));
}
