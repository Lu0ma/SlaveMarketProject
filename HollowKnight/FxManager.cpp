...
void FxManager::Run(const string& _runningAnimation)
{
	...
	/*map<string, function<void()>> _fxs = {

		{"FxDash",[&]() {
		const Vector2f& _position = _direction == 1 ? _pos + Vector2f(200.0f, 0.0f) : _pos - Vector2f(200.0f, 0.0f);
		Fx* _fxdash2 = new Fx("fx" + to_string(index), ShapeData(_position, Vector2f(250.0f, 250.0f), PATH_FX));
		...
		{"FxIsHitten", [&]() {
		const Vector2f& _position = _direction == 1 ? _pos + Vector2f(-25.0f, 0.0f) : _pos + Vector2f(25.0f, 0.0f);
		Fx* _fx = new Fx("fx" + to_string(index), ShapeData(_position, Vector2f(250.0f, 250.0f), PATH_FX2));
